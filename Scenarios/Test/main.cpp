...
#include <Eigen/Dense>
#include "../Common/FluidModel.h"
#include "../Common/TimeStepFluidModel.h"
...
#include "../../Demos/Common/imguiParameters.h"
...
int main( int argc, char **argv )
{
	...
	base = new DemoBase();
	base->init(argc, argv, "FluidSolid test");

	SimulationModel* modelSolid = new SimulationModel();
	modelSolid->init();
	Simulation::getCurrent()->setModel(modelSolid);
	...
	std::cout << "Done GUI model" << std::endl;
...
void buildModel ()
{
	...
	SimulationModel* modelSolid = Simulation::getCurrent()->getModel();
	std::cout << "A1" << std::endl;
	auto temp0 = Simulation::getCurrent();
	std::cout << "A10" << std::endl;
	auto temp1 = temp0->getTimeStep();
	std::cout << "A11" << std::endl;
	temp1->setCollisionDetection(*modelSolid, cd);
...
{
	SimulationModel* modelSolid = Simulation::getCurrent()->getModel();
	SimulationModel::RigidBodyVector& rb = modelSolid->getRigidBodies();
	SimulationModel::ConstraintVector& constraints = modelSolid->getConstraints();

	LOG_INFO << "Initialize solid";
	std::cout << "Initialize solid" << std::endl;
	...
	const std::vector<Vector3r>& verticesBox = rb[rbIndex]->getGeometry().getVertexDataLocal().getVertices();
	const unsigned int nVertBox = static_cast<unsigned int>(verticesBox.size());
	cd->addCollisionBox(rbIndex, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType, verticesBox.data(), nVertBox, Vector3r(100.0, 1.0, 100.0));
	rbIndex++;

	// cube
	std::cout << "Initialize cube" << std::endl;
	rb[rbIndex] = new RigidBody();
	...
	std::cout << "A0" << std::endl;
	const std::vector<Vector3r>& verticesCube = rb[rbIndex]->getGeometry().getVertexDataLocal().getVertices();
	std::cout << "A1" << std::endl;
	const unsigned int nVertCube = static_cast<unsigned int>(verticesCube.size());
	std::cout << "A2" << std::endl;
	cd->addCollisionBox(rbIndex, CollisionDetection::CollisionObject::RigidBodyCollisionObjectType, verticesCube.data(), nVertCube, Vector3r(1.0, 1.0, 1.0));
	std::cout << "A3" << std::endl;
	rbIndex++;

	
	LOG_INFO << "Initialize fluid particles";
	std::cout << "Initialize fluid particles" << std::endl;
