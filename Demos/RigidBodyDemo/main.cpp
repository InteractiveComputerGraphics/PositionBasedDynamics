...
void mouseMove(int x, int y)
{
	...
	{
		rb[selectedBodies[j]].getVelocity() += 5.0f / h * diff;
	}
...
void render ()
{
	...
	{
		MiniGL::drawSphere(bj[i].m_points[0], 1.25f*height, jointColor);
	}
...
void createBodyModel()
{
	...
	bj.resize(numberOfBodies-1);
	for (unsigned int i = 0; i < numberOfBodies-1; i++)
	{
		model.setBallJoint(i, i, i + 1, Eigen::Vector3f((float)i*width + 0.5f*width, 0.0f, 0.0f));
	}
}
